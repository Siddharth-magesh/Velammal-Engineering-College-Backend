...
app.post('/api/update_notifications', verifyAdminAccess('landing_page_access'), async (req, res) => {
    try {
...
app.post('/api/review_notifications_request', verifyAdminAccess('landing_page_access'), async (req, res) => {
    try {
        const { unique_id, reviewed_notes, actions } = req.body;
        if (!unique_id || !actions) {
            return res.status(400).json({ error: 'Missing required fields' });
